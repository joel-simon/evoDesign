import numpy
import warnings
from src.modules.truss.physical_properties import materials, valid_member_name
...
class Member(object):
    ...
    def __init__(self, joint_a, joint_b):
        ...
        self.set_shape("bar", update_props=False)
        self.set_material("cell", update_props=False)
        self.set_parameters(r=.1, update_props=True)
    ...
    def set_force(self, force):
        assert(not numpy.isnan(force))
        self.force = force
        
        if self.force == 0.0:
            self.fos_yielding = 10000
            self.fos_buckling = 10000
        else:
            ...
            if self.fos_buckling <= 0:
                self.fos_buckling = 10000
        self.fos = min(self.fos_yielding, self.fos_buckling)
