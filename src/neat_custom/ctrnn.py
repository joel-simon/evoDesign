"""
The Below code is nearly identical to that of the neat libraty.
Expect for bugs fixed in serial activate. These bugs only arise if adding
input nodes after other nodes and dont effect regular usage.
"""
...
class Network(object):
    ...
    def serial_activate(self, inputs):
        ...
        input_neurons = [n for n in self.neurons if n.type == 'INPUT']

        # print('NUM INPUT NEURONS', len(input_neurons))
    ...
    def parallel_activate(self, inputs=None):
        ...
        current_state = []
        non_input = [ n for n in self.neurons if n.type != 'INPUT' ]
        for n in non_input:
            current_state.append(n.activate())
        # updates all neurons at once
        net_output = []
        for n, state in zip(non_input, current_state):
